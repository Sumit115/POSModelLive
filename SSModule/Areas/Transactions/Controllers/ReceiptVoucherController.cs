...
{
    [Area("Transactions")]
    public class ReceiptVoucherController : BaseTranController<IVoucherRepository, IGridLayoutRepository, ICompositeViewEngine, IWebHostEnvironment>
    {
        private readonly IVoucherRepository _repository;

        public ReceiptVoucherController(IVoucherRepository repository, IGridLayoutRepository gridLayoutRepository, ICompositeViewEngine viewEngine, IWebHostEnvironment webHostEnvironment) : base(repository, gridLayoutRepository, viewEngine, webHostEnvironment)
        {
            _repository = repository;
            TranType = "";
            TranAlias = "V_RC";
            StockFlag = "C";
            FKFormID = (long)Handler.Form.ReceiptVoucher;
            PostInAc = false;
            PageHeading = "Receipt Voucher";
        }
        ...
        public ActionResult Export(string FDate, string TDate)
        {
            ...
            using (XLWorkbook wb = new XLWorkbook())
            {
                ...
                {
                    wb.SaveAs(stream);
                    return File(stream.ToArray(), "application/ms-excel", "Receipt-Voucher-List.xls");
        ...
        [HttpGet]
        [Route("Transactions/ReceiptVoucher/Create/{id?}/{FKSeriesID?}/{isPopup?}")]
        [FormAuthorize(FormRight.Access)]
